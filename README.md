...
全部完整模板（155 个）通过 `g++ -std=c++17 -O2 -fsyntax-only` 语法检查；验证状态与合并时的全局变量重名表见索引末尾两节。
...
- 模板语法检查可自行复现：把代码块存为 `.cpp` 后运行 `g++ -std=c++17 -O2 -fsyntax-only xxx.cpp`。
- 索引末尾列出了各模板的验证状态（已对拍 / 仅编译通过），首次使用前建议先跑样例。
- 发现问题或有补充，欢迎在本仓库提交 Issue。

维护：[@chromoany](https://github.com/chromoany)　最后更新：2026-09-22
